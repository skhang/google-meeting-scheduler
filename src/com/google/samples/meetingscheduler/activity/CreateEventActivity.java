...
import com.google.samples.meetingscheduler.util.CalendarServiceBuilder;
...
public class CreateEventActivity extends Activity {
    ...
    saveButton.setOnClickListener(new OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        if (OAuthManager.getInstance().getAuthToken() == null) {
          OAuthManager.getInstance().doLogin(false, CreateEventActivity.this,
   ...
   * @param where Event's location.
   * @param description Event's description
   * @param sendEventNotifications Whether or not to send a notification to
   *        attendees.
   */
  private void createEvent(final String title, final String where, final String description,
      final boolean sendEventNotifications, final int tryNumber) {
    // Show a progress bar while the common free times are computed.
    if (progressBar == null) {
      progressBar =
          ProgressDialog.show(CreateEventActivity.this, null,
              CreateEventActivity.this.getString(R.string.create_event_wait_text), true);
    }
    ...
    new Thread(new Runnable() {
      ...
      public void run() {
        ...
        try {
          Calendar service =
              CalendarServiceBuilder.build(OAuthManager.getInstance().getAuthToken());
          ...
          service.events().insert("primary", newEvent).setSendNotifications(sendEventNotifications)
              .execute();
          setResult(RESULT_OK);
        } catch (IOException e) {
          if (e instanceof HttpResponseException) {
            ...
            if (statusCode == 401 && (tryNumber - 1) > 0) {
              ...
              OAuthManager.getInstance().doLogin(true, CreateEventActivity.this,
                  new OAuthManager.AuthHandler() {
  ...
  private void getParameters() {
    ...
    startDate =
        new DateTime(intent.getLongExtra(START_DATE, java.util.Calendar.getInstance()
            .getTimeInMillis()), 0);
    endDate =
        new DateTime(intent.getLongExtra(END_DATE, java.util.Calendar.getInstance()
            .getTimeInMillis() + 3600000), 0);
  }
